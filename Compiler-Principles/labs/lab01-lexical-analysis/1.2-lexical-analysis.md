...
本关任务：实现 PL/0 语言的词法分析器，输入源程序，并对其进行扫描和分解，识别出单词符号。

PL/0 语言词法规则定义如下：
1. 唯一的数据类型是整型。
2. 算符和界符有：`+`, `-`, `*`, `/`, `>`, `>=`, `<`, `<=`, `=`, `#`, `.`, `(`, `)`, `,`, `;`, `:=`
3. 标识符以字母开头，后跟字母和/或数字组成的字符串。
4. 关键字作为保留字，包括：`const`, `var`, `procedure`, `call`, `begin`, `end`, `if`, `then`, `while`, `do`, `odd`
...
![词法分析状态转换图](assets/1.2-lexical-analysis/lexical_state_transition.png "词法分析状态转换图")

单词符号输出形式为 `(种别编码, 单词符号自身值)`：
...
- `int error(int ERROR_CODE_TYPE)`：识别词法错误。
- `string GETSYM()`：词法分析。

### 编程要求

根据提示，在右侧代码编辑器窗口补充适当的代码，完成以下内容：
1. 标识符的识别
2. 关键字和算符界符的识别
3. 整型常数的识别
4. 常数表的组织
...
平台会对你编写的代码进行测试，你必须通过全部 5 个测试集才能通关。
